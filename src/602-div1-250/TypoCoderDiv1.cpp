...
class TypoCoderDiv1 {
    ...
    int dp[55][2300];
public:
    int getmax(vector<int> _D, int _X) {
        D = _D, X = _X;
        int n = SZ(D);
        MSET(dp, -1);
        dp[0][X] = 0;
        REP(i, n) REP(j, 2200) if(dp[i][j] >= 0){
            chmax(dp[i+1][max(0,j-D[i])], dp[i][j]);
            if(j + D[i] < 2200) chmax(dp[i+1][j+D[i]], dp[i][j]);
            if(j + D[i] >= 2200 && j + D[i] - D[i+1] < 2200) chmax(dp[i+2][max(0, j+D[i]-D[i+1])], dp[i][j] + 2);
        }
        int res = 0;
        REP(i, 2200) chmax(res, dp[n][i]);
        for(int i = max(0, 2200-D[n-1]); i < 2200; i++) chmax(res, dp[n-1][i] + 1);
        return res;
    }
};
