...
public:
   double getAverage( vector <string> ratings ) {
	   sc.clear();
	   REP(i, 20) table[i].clear();

	   string s = "";
	   REP(i, SZ(ratings)) s += ratings[i];
	   stringstream ss;
	   ss << s;
	   string elem;
	   while(ss >> elem){
	   ...
	   int room = (SZ(sc)+19)/20;
	   int regs = 0;
	   for(int i = 0; i < SZ(sc); regs++){
		   for(int k = 0; k < room && i < SZ(sc); k++, i++){
			   table[regs].PB(sc[i]);
			   if(sc[i] == elly) pos = regs;
		   }
	   }

	   REP(i, regs){
		   double tmp = 0.0;
		   REP(j, SZ(table[i])) tmp += table[i][j];
		   tmp /= SZ(table[i]);
		   expe[i] = tmp;
	   }
	   double res = 0.0;
	   REP(i, SZ(table[0])){
		   if(i >= SZ(table[regs-1])){
			   if(pos == regs-1) continue;
			   double tmp = 0.0;
			   REP(j, regs-1) if(j != pos) tmp += expe[j];
			   tmp += elly;
			   res += (tmp/(regs-1));
		   }
		   else{
			  double tmp = 0.0;
			  REP(j, regs) if(j != pos) tmp += expe[j];
			   tmp += elly;
			   res += (tmp/regs);
		   }
	   }

	   if(pos == regs-1) return res/SZ(table[regs-1]);
	   else return res/room;
