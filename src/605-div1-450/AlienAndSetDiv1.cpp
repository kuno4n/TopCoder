...
public:
    int getNumber(int _N, int _K) {
        ...
        MSET(dp, 0);
        dp[0][0][0] = 1;
        REP(i, 2*N) REP(j, min(N+1, i+1)) REP(k, (1<<K)){
            int bits = bitcnt(k);
            if(bits > j) continue;
            // int pos = 31 - __builtin_clz(j); // 0 -> -1, 1 -> 0, 2 -> 1, 4 -> 2 etc.
            if(j == 0){
                add(dp[i+1][1][K==1 ? 0 : 1], dp[i][j][k] * 2);
            }
            else{
                int nxtk = k << 1;
                nxtk &= (1<<(K-1)) - 1;
                if(bits < j){
                    add(dp[i+1][j-1][nxtk], dp[i][j][k]);
                }
                add(dp[i+1][j+1][nxtk+1], dp[i][j][k]);
            }
        } 
        return dp[2*N][0][0];
    }
