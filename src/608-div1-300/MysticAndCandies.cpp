...
public:
    int minBoxes(int _C, int _X, vector<int> _low, vector<int> _high) {
        C = _C, X = _X, low = _low, high = _high;
        sort(ALL(high));
        int sum = 0;
        int res = SZ(high);
        REP(i, SZ(high)){
            sum += high[i];
            if(C-sum >= X) chmin(res , SZ(high)-i-1);
        }
        sort(ALL(low));
        reverse(ALL(low));
        sum = 0;
