...
class BlackBoxDiv2 {
    ...
    int cnt(int h, int w){
        ...
        for(int i = 2; i <= h; i++) for(int j = 2; j <= w; j++){
            for(int a = 1; a < j; a++) add(dp[i][j], (dp[i-1][a] * C[j][a]) % MOD * modPow(2, a));
            add(dp[i][j], dp[i-1][j] * (modPow(2, j)-1+MOD) % MOD);
        }
        return dp[h][w];
    }
...
public:
    ...
    makepas();
        int h = 0;
        int w = 0;
        REP(i, SZ(front)) h += front[i] == 'B';
        REP(i, SZ(side)) w += side[i] == 'B';
        return cnt(h, w);
    }
};
