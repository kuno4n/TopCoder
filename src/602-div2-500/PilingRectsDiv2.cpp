...
class PilingRectsDiv2 {
    ...
    int limit;
public:
    int getmax(vector<int> _X, vector<int> _Y, int _limit) {
        X = _X, Y = _Y, limit = _limit;
        int n = SZ(X);
        int res = -1;

        vector<pair<int, int> > v;
        REP(i, n) v.push_back(make_pair(max(X[i], Y[i]), min(X[i], Y[i])));
        sort(ALL(v));
        reverse(ALL(v));
        REP(i, n){
            int x = v[i].first;
            VI w;
            REP(j, i+1) w.push_back(v[j].second);
            sort(ALL(w));
            reverse(ALL(w));
            REP(j, SZ(w)) if(x * w[j] >= limit) chmax(res, j+1);
        }
        return res;
    }
};
