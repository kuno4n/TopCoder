...
public:
   int countsets( int L, int H, int K ) {
	...
	for(int i = 0; i <= L; i++) add(sum1, C[H+1][K]);
	
	for(int i = 1; i <= L; i++) for(int j = 1; j <= H; j++){
		if(gcd(i, j) > 1) continue;
		for(int x = 0; x <= L; x++) add(sum2, C[min((L-x)/i, H/j) + 1][K]);
	}
	return (sum1+sum2*2) % MOD;
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			int received__            = SpacetskE().countsets(L, H, K);
