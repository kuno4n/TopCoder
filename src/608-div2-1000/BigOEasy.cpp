...
void rec(int a, int from){
    visited[a] = true;
    REP(i, n) if(graph[a][i] == 'Y'){
        if(i == from) cnt++;
        if(!visited[i]) rec(i, from);
    }
}

class BigOEasy {
public:
    string isBounded(vector<string> _graph) {
        graph = _graph;
        n = SZ(graph);
        REP(i, n){
            cnt = 0;
