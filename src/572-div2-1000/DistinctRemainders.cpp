...
LL rec(int sum, int part){
	if(sum < 0) return 0;
	if(mem[sum][part] >= 0) return mem[sum][part];
	if(part == 1) return mem[sum][part] = 1;
	
	LL res = 0;
	int part2 = part-1;
	for(int i = 1; i <= sum; i++){
		int sum2 = sum - i*part2;
		res += rec(sum2, part2);
		res %= MOD;
	}
	return mem[sum][part] = res;
}

class DistinctRemainders {
public:
   int howMany( long long N, int M ) {
	REP(i, 50) REP(j, 20)  mem[i][j] = -1;
	OUT(rec(3,2));
	REP(i, 4) REP(j, 3) {OUT(i);OUT(j);OUT(mem[i][j]);}
